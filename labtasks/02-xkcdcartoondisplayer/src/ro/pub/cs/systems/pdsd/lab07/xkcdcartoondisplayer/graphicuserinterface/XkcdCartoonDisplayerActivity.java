...
public class XkcdCartoonDisplayerActivity extends Activity {
	...
	private class XkcdCartoonUrlButtonClickListener implements
	...
	private class XkcdCartoonDisplayerAsyncTask extends
			AsyncTask<String, Void, XkcdCartoonInfo> {
		...
		protected XkcdCartoonInfo doInBackground(String... urls) {
			...
			// TODO: exercise 5a)
			// 1. obtain the content of the web page (whose Internet address is
			// stored in urls[0])
			...
			try {
				result = client.execute(httpXkcdGet, responseHandlerGet);
			...
			// - cartoon content: get the input stream attached to the url and
			...
			// * attach the previous button a click listener with the address
			// attached
			// - next cartoon address
			// * get the first tag whole rel attribute equals "next"
			// * get the href attribute of the tag
			// * prepend the value with the base url: http://www.xkcd.com
			// * attach the next button a click listener with the address
			// attached
			...
			if (result != null) {
				...
				Element divTagIdComic = htmlTag.getElementsByAttributeValue(
						Constants.ID_ATTRIBUTE, Constants.COMIC_VALUE).first();
				String cartoonInternetAddress = divTagIdComic.getElementsByTag(
						Constants.IMG_TAG).attr(Constants.SRC_ATTRIBUTE);

				try {
					String cartoonUrl = Constants.HTTP_PROTOCOL
							+ cartoonInternetAddress;
					HttpGet httpCartoonGet = new HttpGet(cartoonUrl);
					HttpResponse httpGetResponse = client
							.execute(httpCartoonGet);
					HttpEntity httpGetEntity = httpGetResponse.getEntity();
					if (httpGetEntity != null) {
						xkcdCartoonInfo.setCartoonContent(BitmapFactory
								.decodeStream(httpGetEntity.getContent()));
					}
				...
				Element aTagRelPrev = htmlTag.getElementsByAttributeValue(
						Constants.REL_ATTRIBUTE, Constants.PREVIOUS_VALUE)
						.first();
				String previousCartoonInternetAddress = Constants.XKCD_INTERNET_ADDRESS
						+ aTagRelPrev.attr(Constants.HREF_ATTRIBUTE);
				previousButton
						.setOnClickListener(new XkcdCartoonUrlButtonClickListener(
								previousCartoonInternetAddress));

				Element aTagRelNext = htmlTag.getElementsByAttributeValue(
						Constants.REL_ATTRIBUTE, Constants.NEXT_VALUE).first();
				String nextCartoonInternetAddress = Constants.XKCD_INTERNET_ADDRESS
						+ aTagRelNext.attr(Constants.HREF_ATTRIBUTE);

				nextButton
						.setOnClickListener(new XkcdCartoonUrlButtonClickListener(
								nextCartoonInternetAddress));
			}
		...
		protected void onPostExecute(XkcdCartoonInfo xkcdCartoonInfo) {
			...
			// TODO: exercise 5b)
			// map each member of xkcdCartoonInfo object to the corresponding
			// widget
			...
			if (xkcdCartoonInfo != null) {
				xkcdCartoonTitleTextView.setText(xkcdCartoonInfo
						.getCartoonTitle());
				xkcdCartoonImageView.setImageBitmap(xkcdCartoonInfo
						.getCartoonContent());
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new XkcdCartoonDisplayerAsyncTask()
				.execute(Constants.XKCD_INTERNET_ADDRESS);
	}
